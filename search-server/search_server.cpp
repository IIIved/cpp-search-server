...
#include "string_processing.h"
    ...
    int SearchServer::SearchServer::GetDocumentCount() const {
    ...
    for (auto& [word, value] : word_to_document_freqs_) {
    ...
    void SearchServer::RemoveDocument(int document_id){
        vector<string> temp_cont;
        documents_.erase(document_id);
        for (const auto& [word, value] : word_to_document_freqs_){
            if(value.count(document_id))
                temp_cont.push_back(word);
        }
        for (const string& word : temp_cont){
